...
class line_measure:
    ...
    def draw(self):
        self.measure_surface = pygame.Surface((self.width, abs(self.measure_shift )), pygame.SRCALPHA)
        # self.measure_surface.fill(self.colors['test'])
        self.arrow = arrow((self.arrow_height , self.measure_shift ), self.measure_surface,
                           self.colors, True,
        ...
        if (self.measure_shift > 0):
            ...
            if (self.angle_rotate == 0):
                self.blit_point = (self.end_point[0], self.end_point[1] + ((self.start_point[1] - self.end_point[1]) - self.measure_shift))
            ...
            pygame.draw.line(self.measure_surface, self.colors['border'],
                             (self.width - 5, abs(self.measure_shift)),
                             (self.width - 5, abs(00)),
                             self.border_size - 1)

            pygame.draw.line(self.measure_surface, self.colors['border'],
                             (0, abs(self.measure_shift )),
                             (0, abs(0)),
                             self.border_size - 1)
            ...
            self.text_point =  (self.width / 2 -  self.text_surface.get_width() / 2,
                                                     abs(self.measure_shift ) - self.arrow_height - self.font_size)
            self.measure_surface.blit( self.text_surface, self.text_point)
